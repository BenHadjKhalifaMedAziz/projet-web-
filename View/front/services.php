...
if ($type == 'art') {
	if (isset($_GET["search"])) {
		$search = $_GET["search"];
		$products = $productController->getArtListSearch($order, $search);
	} else {
		$products = $productController->getArtList($order);
	}
} else {
	if (isset($_GET["search"])) {
		$search = $_GET["search"];
		$products = $productController->getCulturalListSearch($order, $search);
	} else {
		$products = $productController->getCulturalList($order);
	}
}
...
{
	if ($type != $item) {
		return '<li class=""><a href="services.php?type=' . $item . '">' . $name . '</a></li>';
	} else {
		return '<li class="active">
				<a href="services.php?type=' . $item . '">
...
function getSortItem($type, $order, $item, $name)
{
	if ($order != $item) {
		return '<a class="btn btn-dark mx-2" href="services.php?type=' . $type . '&order=' . $item . '">' . $name . '</a></li>';
	} else {
		return '<a class="btn btn-success mx-2" href="services.php?type=' . $type . '&order=' . $item . '">' . $name . '</a></li>';
	}
}
?>
								...
								<div class="col">
									...
									<div class="row">
										<div class="col-md-4 d-flex">
											<div class="mt-2">Filtre Prix : </div>
											<?php
											echo getSortItem($type, $order, 'asc', 'Asc');
											echo getSortItem($type, $order, 'desc', 'Desc');
											?>
										</div>
										<div class="col-md-8">
											<form action="services.php" method="get">
												<input type="hidden" name="type" value="<?php echo $type ?>" />
												<input name="search" style=" display:unset; width:200px" class="form-control" type="text" placeholder="Search by name">
